...
void ILI9341_SetCursorPosition(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
  ...
  ILI9341_SendCommand16 (ILI9341_COLUMN_ADDR);
  ILI9341_SendData16(x1>>8);
  ILI9341_SendData16(x1 & 0xFF);
  ILI9341_SendData16(x2>>8);
  ILI9341_SendData16(x2 & 0xFF);

  ILI9341_SendCommand16 (ILI9341_PAGE_ADDR);
  ILI9341_SendData16(y1>>8);
  ILI9341_SendData16(y1 & 0xFF);
  ILI9341_SendData16(y2>>8);
  ILI9341_SendData16(y2 & 0xFF);
  ILI9341_SendCommand16 (ILI9341_GRAM);
}
//4. Initialise function
void ILI9341_Init(void)
 {
   ILI9341_SendCommand16 (ILI9341_RESET); // software reset comand
   HAL_Delay(100);
   ILI9341_SendCommand16 (ILI9341_DISPLAY_OFF); // display off
   //------------power control------------------------------
   ILI9341_SendCommand16 (ILI9341_POWER1); // power control
   ILI9341_SendData16   (0x26); // GVDD = 4.75v
   ILI9341_SendCommand16 (ILI9341_POWER2); // power control
   ILI9341_SendData16   (0x11); // AVDD=VCIx2, VGH=VCIx7, VGL=-VCIx3
   //--------------VCOM-------------------------------------
   ILI9341_SendCommand16 (ILI9341_VCOM1); // vcom control
   ILI9341_SendData16   (0x35); // Set the VCOMH voltage (0x35 = 4.025v)
   ILI9341_SendData16   (0x3e); // Set the VCOML voltage (0x3E = -0.950v)
   ILI9341_SendCommand16 (ILI9341_VCOM2); // vcom control
   ILI9341_SendData16   (0xbe);

   //------------memory access control------------------------
   ILI9341_SendCommand16 (ILI9341_MAC); // memory access control
   ILI9341_SendData16(0x48);

   ILI9341_SendCommand16 (ILI9341_PIXEL_FORMAT); // pixel format set
   ILI9341_SendData16   (0x55); // 16bit /pixel
   ...
   ILI9341_SendData16   (0x00); // x0_LOW----0
   ILI9341_SendData16   (0x00); // x1_HIGH---240
   ILI9341_SendData16   (0xEF); // x1_LOW----240
   ILI9341_SendCommand16 (ILI9341_PAGE_ADDR); // page address set
   ILI9341_SendData16   (0x00); // y0_HIGH---0
   ILI9341_SendData16   (0x00); // y0_LOW----0
   ...
   ILI9341_SendData16   (0x07);
   //-----------------display------------------------
   ILI9341_SendCommand16 (ILI9341_DFC); // display function control
   //Set the scan mode in non-display area
   //Determine source/VCOM output in a non-display area in the partial display mode
   ILI9341_SendData16   (0x0a);
   //Select whether the liquid crystal type is normally white type or normally black type
   //Sets the direction of scan by the gate driver in the range determined by SCN and NL
   //Select the shift direction of outputs from the source driver
   //Sets the gate driver pin arrangement in combination with the GS bit to select the optimal scan mode for the module
   //Specify the scan cycle interval of gate driver in non-display area when PTG to select interval scan
   ILI9341_SendData16   (0x82);
   // Sets the number of lines to drive the LCD at an interval of 8 lines
   ILI9341_SendData16   (0x27);
   ILI9341_SendData16   (0x00); // clock divisor

   ILI9341_SendCommand16 (ILI9341_SLEEP_OUT); // sleep out
   HAL_Delay(100);
   ILI9341_SendCommand16 (ILI9341_DISPLAY_ON); // display on
   HAL_Delay(100);
   ILI9341_SendCommand16 (ILI9341_GRAM); // memory write
   HAL_Delay(5);
 }
...
void ILI9341_setRotation(uint8_t rotate)
{
	switch(rotate)
	{
		case 1:
			rotationNum = 1;
            ILI9341_SendCommand16(ILI9341_MEMCONTROL);
            ILI9341_SendData16(ILI9341_MADCTL_MY | ILI9341_MADCTL_BGR);
			break;
		case 2:
			rotationNum = 2;
            ILI9341_SendCommand16(ILI9341_MEMCONTROL);
            ILI9341_SendData16(ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR);
			break;
		case 3:
			rotationNum = 3;
            ILI9341_SendCommand16(ILI9341_MEMCONTROL);
            ILI9341_SendData16(ILI9341_MADCTL_MX | ILI9341_MADCTL_BGR);
			break;
		case 4:
			rotationNum = 4;
            ILI9341_SendCommand16(ILI9341_MEMCONTROL);
            ILI9341_SendData16(ILI9341_MADCTL_MX | ILI9341_MADCTL_MY | ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR);
			break;
		default:
			rotationNum = 1;
            ILI9341_SendCommand16(ILI9341_MEMCONTROL);
